...
class BrowseViewController: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource, UITextFieldDelegate {
    ...
    @objc func textFieldDidChange(_ textField: UITextField) {
            ...
            Task {
                do {
                    let articles = try await NewsAPIService.shared.searchNews(withQuery: query)
                    await MainActor.run {
                        self.newsArticlesDict["searchResults"] = articles
                        self.activeCategory = "searchResults"
                        self.collectionView.reloadData()
                    }
                } catch {
    ...
    private func loadNews(forCategory category: String) {
        Task {
            do {
                let articles = try await NewsAPIService.shared.loadNews(forCategory: category)
                updateUI(with: articles, category: category)
            } catch {
    ...
    private func searchNews(withQuery query: String) {
        Task {
            do {
                let articles = try await NewsAPIService.shared.searchNews(withQuery: query)
                updateUI(with: articles, category: "searchResults")
            } catch {
    ...
    private func updateUI(with articles: [NewsArticle], category: String) {
        Task { @MainActor in
            self.newsArticlesDict[category] = articles
            self.collectionView.reloadData()
        }
    }
